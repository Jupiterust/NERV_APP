...
| 项 | 状态 | 位置 |
|---|---|---|
| P0-1 协议缓冲区扩容 | ✅ 已改，已验证 | `Protocol/Custom_Protocol/General_Protocol.c` |
| P0-2 驱动层二进制追加 / 定位写 | ✅ 已加，已验证 | `Driver/bsp_sdio.c` + `.h` |
| P0-3 长文件名 LFN | ✅ 本来就开着，已验证 | `FatFt/ffconf.h`：`_USE_LFN 1` + `_CODE_PAGE 437` |
| 文件服务 + 9 条命令 | ✅ 已加 | `Function/file_service.h` / `.c` |
| 接入（Headfile / switch / 主循环 / 两个工程文件） | ✅ 已接 | 见第六节 |
| 真卡验证 | 🟡 **部分做了** | T-01/T-04/T-10/T-11 已在真机上跑通（开始记录→停止→查大小，数字对得上），T-02/03/05~09/12~18 还没测，见第九节自检清单 |
| 操作日志落卡（附加功能，不属于 file_service） | ✅ 已加，✅ **已在真机验证** | `Function/Op_log_function.h` / `.c`，见第十一节 |

**编译状态**：`file_service` 这部分本来是对着真实接口手写、没编译过的（当时这台机器没有 armcc/gcc）；第十一节的操作日志落卡是后来单独加的，已经在真实硬件上 build、烧录、跑通，细节见第十一节。**`file_service` 剩下没测的命令，第一件事仍然是在 Keil 里跑一遍第七节的测试报文表。**
...
## 十一、附加功能：操作日志落 TF 卡（`Function/Op_log_function.h` / `.c`）

**这是一个独立于 `file_service` 的功能，文件不一样、触发方式不一样，别混在一起改。** `file_service` 管的是上位机主动下发的文件读写命令；这里管的是**把已有的操作日志（RX/TX 报文）自动追加写到卡上**，两者共用同一张 TF 卡，互不干扰。

**状态：✅ 已经在真机上跑通**，包括中途主动复位、文件不删继续写这种边界情况都验证过（见下面"已验证的事实"）。

### 它是什么

`Function/Op_log_function.c` 本来就有 6 个 RAM 环形缓冲区，记录自定义协议 / Modbus RTU / Modbus ASCII 收发的每一帧（详见 `Op_log_function.h` 顶部注释）。这个附加功能在此基础上加了一条**独立的落盘队列**：每次往 RAM 环里记一笔的同时，也拷贝一份进这条队列；主循环里空闲时把队列里攒的记录格式化、追加写进 `OPLOG.TXT`。

设计上的核心约束：**记一笔（`oplog_add()`）只做内存拷贝，绝不碰卡**；真正的文件 I/O 全部挪到 `oplog_sd_poll()`，在主循环里 `Protocol_Route()` 之后调用——这时候这一帧的应答已经交给串口中断发出去了，写卡不会拖慢应答。没插卡时静默跳过，不影响任何现有功能。

### 现场怎么改 —— 单开一张表

| 现场要求 | 改哪个 |
|---|---|
| 文件名不同 | `Op_log_function.h`【1】节的 `OPLOG_SD_FILENAME` |
| 不要表头 | `OPLOG_SD_WRITE_HEADER` 改 0 |
| 表头文字要改 | `OPLOG_SD_HEADER_TEXT` |
| 要更及时地落盘（不要攒够 8 条才写） | 调小 `OPLOG_SD_FLUSH_COUNT`，代价是开关文件次数变多 |
| 要更久才落一次盘（减少写卡次数） | 调大 `OPLOG_SD_FLUSH_INTERVAL_S` |
| 队列老是丢记录（`oplog_sd_get_dropped()` 查出来的数一直涨） | 调大 `OPLOG_SD_QUEUE_DEPTH`，或者调小上面两项让落盘更勤 |
| 干脆不要这个功能了 | `OPLOG_SD_ENABLE` 改 0，落盘代码和它占的 RAM 全部编译掉，`main.c` 不用改 |
| 只想记某一种协议（比如只要 Modbus） | 见下面"如果要改的话"，这条不是纯 `#define` 能解决的 |

**⚠️ 跟 `file_service` 不一样的地方**：`file_service` 的原则是"只改 `.h`，不动 `.c`"；这个功能里**写入卡的那一行文本长什么样，是 `.c` 里的代码决定的，不是 `#define`**。原因是它复用了 `oplog_format()`——这个函数同时服务于调试串口打印（`oplog_dump_all()`）和落盘写文件，两者必须是同一套格式，不能各自维护一份。

**如果现场要求改格式**（比如列的顺序、分隔符、要不要十六进制）：

- 分隔符/列的顺序/整体排版 → 改 `Op_log_function.c` 的 `oplog_format()` 函数本身，就一个 `snprintf` 调用，格式串是：
  ```c
  "%04d-%02d-%02d %02d:%02d:%02d | %s | %-6s | %u | "
  ```
  改这一行连打印到调试串口的格式也会跟着变，这是故意的（两处要求同一个格式，见上面）。
- 报文正文是十六进制还是原始字符 → `Op_log_function.h`【1】节的 `OPLOG_FORCE_HEX`（0=自动：RTU 转十六进制，自定义协议/ASCII 原样显示；1=一律十六进制）。
- 时间戳的显示时区 → `OPLOG_TZ_OFFSET_HOUR`。
- 只想记特定协议：目前是"能记的都记"（三种协议 × 收发方向共 6 类，全部落盘）。要精简的话，最简单的做法不是改 `oplog_add()`（那是 8 个记录点共用的入口，改了影响面太大），而是在 `oplog_sd_enqueue()`（`Op_log_function.c` 里，`oplog_add()` 调它入队之前）加一行按 `e->proto` 过滤，不想要的直接不入队。

### 已验证的事实（真机测试跑出来的，不是猜的）

- 连续发送自定义协议 + Modbus RTU + Modbus ASCII 命令，队列每攒够 8 条或者超过 5 秒都能正确触发落盘，写入的条目数和实际发送的报文数对得上。
- **中途主动复位、`OPLOG.TXT` 没删**，复位前后日志是连续追加的，**没有出现重复表头**——这是修过一次的 bug（`s_sd_header_written` 只在 RAM 里，原来每次重启都不知道文件已经有表头，会在文件中间再插一条；现在改成开机第一次落盘前先查一下文件大小，非空就跳过表头）。
- 时间戳显示的是**设备 RTC 的值，不是电脑当前时间**。如果 RTC 从没被设置过（没收到过 `0x0105`/`MB_HREG_SET_TIME`），文件里的时间戳会是设备自己认的一个旧值，不是真实当前时间——现场评测前建议先发一次设置时间的命令，不然日志时间戳对不上评委的表。这不是这个功能的 bug，是 RTC 配置的问题。

### 现场如果出问题，先看这个

`Op_log_function.c` 的 `oplog_sd_poll()` 里目前还留着几行 `TODO(debug)` 标记的临时 `printf`（`[OPLOG-SD] triggered/...`），专门用来排查"卡上为什么没生成/没更新日志"这类问题——如果现场真出这种情况，先看调试串口有没有打这几行，比瞎猜快得多。确认长期稳定之后可以删掉，删的时候把 `bool wr_ok` 这个局部变量一起清理掉（只有这几行调试打印在用）。
